...
fn test_fund_wrong_caller() {
    ...
    rewards_dispatcher.fund(amount: cfg.default_fund, use_locked_rewards: false);
}
...
fn test_fund_no_points() {
    ...
    let amount = cfg.default_fund;
    cheat_caller_address_once(contract_address: cfg.token_address, caller_address: cfg.funder);
    token_dispatcher.approve(spender: cfg.rewards_contract, amount: amount.into());

    cheat_caller_address_once(contract_address: cfg.rewards_contract, caller_address: cfg.funder);
    rewards_dispatcher.fund(:amount, use_locked_rewards: false);
}
...
fn test_lock_rewards() {
    ...
    let locked_rewards = rewards_dispatcher.get_locked_rewards();
    assert!(locked_rewards == fund_amount / 2);
    ...
    rewards_dispatcher.lock_rewards(:points, :reward_cycle);
    let locked_rewards = rewards_dispatcher.get_locked_rewards();
    assert!(locked_rewards == fund_amount);
}

#[test]
#[should_panic(expected: "Can only be called by the staking contract")]
fn test_lock_rewards_wrong_caller() {
    let cfg = memecoin_staking_test_setup();
    let rewards_dispatcher = IMemeCoinRewardsDispatcher { contract_address: cfg.rewards_contract };

    rewards_dispatcher.lock_rewards(points: 0, reward_cycle: 0);
}

#[test]
#[should_panic(expected: "Reward cycle does not exist")]
fn test_lock_rewards_invalid_cycle() {
    let cfg = memecoin_staking_test_setup();
    let rewards_dispatcher = IMemeCoinRewardsDispatcher { contract_address: cfg.rewards_contract };
...
#[test]
#[should_panic(expected: "Lock points exceeds cycle points")]
fn test_lock_rewards_points_exceeds_cycle_points() {
    let cfg = memecoin_staking_test_setup();
    let staker_address = cfg.staker_address;
    let rewards_dispatcher = IMemeCoinRewardsDispatcher { contract_address: cfg.rewards_contract };
...
fn test_fund_locked_rewards() {
    ...
    let mut spy = spy_events();
    cheat_caller_address_once(contract_address: cfg.rewards_contract, caller_address: cfg.funder);
    rewards_dispatcher.fund(amount: 0, use_locked_rewards: true);
...
#[test]
#[should_panic(expected: "Can't fund with locked rewards with nonzero amount")]
fn test_fund_using_locked_rewards_nonzero_amount() {
    let cfg = memecoin_staking_test_setup();
    let rewards_dispatcher = IMemeCoinRewardsDispatcher { contract_address: cfg.rewards_contract };

    cheat_caller_address_once(contract_address: cfg.rewards_contract, caller_address: cfg.funder);
    rewards_dispatcher.fund(amount: 1, use_locked_rewards: true);
}
