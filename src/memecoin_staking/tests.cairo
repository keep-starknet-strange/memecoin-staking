...
use memecoin_staking::test_utils::{
    ...
    load_value, staking_contract_test_setup, verify_stake_info,
};
...
#[test]
fn test_stake() {
    let cfg = staking_contract_test_setup();
    let token_address = cfg.token_address;
    let staking_dispatcher = IMemeCoinStakingDispatcher { contract_address: cfg.staking_contract };

    let staker_balance: Amount = STAKER_SUPPLY;

    let amount: Amount = staker_balance / 2;
    let stake_duration = StakeDuration::OneMonth;
    cheat_and_approve(
        :token_address, approver: cfg.staker_address, spender: cfg.staking_contract, :amount,
    );
